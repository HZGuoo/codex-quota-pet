...
用户主动点击“检查更新”时，应用会向 GitHub Releases API 发送普通 HTTPS 请求，并只读取最新版本号和发布页地址。项目没有自己的更新服务器，也不会在后台自动检查更新。

## 观测数据导出

`CodexQuotaPetCore` 和 `codex-observe` 可以导出 JSON 或 CSV。导出内容仅包括额度窗口、聚合 Token 数量、同步时间和各任务状态的数量，不包含提示词、回复正文、认证令牌、代理凭据或完整任务标识。
